...
# Emit `KEY=value`, or `#KEY=` when the value is empty. An empty string fails
# schema validation where an absent key falls back to its default, so writing
# `PUBLIC_URL=` produces a file the app refuses to boot from. That is the normal
# outcome of a watch-only setup, which has no tunnel and no YouTube key.
opt() {
  if [ -n "$2" ]; then printf '%s=%s\n' "$1" "$2"; else printf '#%s=\n' "$1"; fi
}

# Only a real origin yields a valid redirect URI; "/oauth/tiktok/callback" on
# its own is not a URL and fails validation the same way.
if [ -n "${PUBLIC_URL}" ]; then
  REDIRECT_URI="${PUBLIC_URL}/oauth/tiktok/callback"
else
  REDIRECT_URI=""
fi
...
PORT=${PORT}
HOST=127.0.0.1
$(opt PUBLIC_URL "${PUBLIC_URL}")

$(opt YOUTUBE_API_KEY "${YOUTUBE_API_KEY}")
$(opt YOUTUBE_CHANNEL_ID "${CHANNEL_ID}")
$(opt WEBSUB_SECRET "${WEBSUB_SECRET}")
...
# Filled in after you create the TikTok app - set-tiktok-app fills these in
# place, so leave them commented rather than empty.
#TIKTOK_CLIENT_KEY=
#TIKTOK_CLIENT_SECRET=
$(opt TIKTOK_REDIRECT_URI "${REDIRECT_URI}")
...
$(opt EXPECTED_TIKTOK_USERNAME "${EXPECTED_TIKTOK_USERNAME}")

$(opt LEGAL_ENTITY_NAME "${LEGAL_ENTITY_NAME}")
$(opt LEGAL_CONTACT_EMAIL "${LEGAL_CONTACT_EMAIL}")

$(opt REVIEW_TOKEN "${REVIEW_TOKEN}")
LOG_LEVEL=info

# Used by setup.sh only.
$(opt NGROK_DOMAIN "${NGROK_DOMAIN}")
$(opt YT_HANDLE "${YT_HANDLE}")
ENVEOF
chmod 600 "$ENV_FILE"
